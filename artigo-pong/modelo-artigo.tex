...
\titulo{Construção de um plataforma de jogo em Processing com integração ao Arduino.}
\tituloestrangeiro{}
\autor{
...
The current project aimed to create a game plataform that includes the game "Pong", developed by Atari Inc. in 1972, adapting it to newer tecnologies. For it's development, the Processing programming language was utilized to create the game itself, and it is integrated with an Arduino UNO microcontroller, which collects information from potentiometers ans buttons, coupled to controls that were 3D printed, and sending it serially to the game, which will process the gathered information to perform actions in game such as moving the paddles or pausing. In summary, the project is a mixed composition of Hardware and Software that aims to run the game "Pong" on a computer.

      \vspace{\onelineskip}

      \noindent
      \textbf{Keywords}: Arduino. Processing. Hardware. Electronic Game.
   \end{otherlanguage*}
\end{resumoumacoluna}
